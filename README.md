...
| ---------------------- | -------------------------------------------------------------------------------------------------------------- |
| `fcg-identity`         | Identidade, autenticação, refresh token, cadastro de Doador, provisionamento de GestorONG e perfis de domínio. |
| `fcg-campaigns`        | Gestão de campanhas, painel público de transparência e atualização idempotente do valor arrecadado.            |
| `fcg-donations`        | Recebimento de intenções de doação e publicação de eventos de doação.                                          |
| `fcg-donation-worker`  | Consumo de eventos de doação, processamento e atualização do status da doação.                                 |
| `fcg-audit-logs`       | Consumo de eventos explícitos de auditoria e persistência em MongoDB.                                          |
| `fcg-solidarity-web`   | Interface web da plataforma.                                                                                   |
| `fcg-solidarity-infra` | Ambiente integrado, Kubernetes, observabilidade, Keycloak, Kafka, MongoDB e Terraform Azure.                   |
| `fcg-pipelines`        | Workflows reutilizáveis de CI/CD consumidos pelos repositórios da plataforma.                                  |

## 2. Como subir localmente

O ambiente integrado deve ser executado pelo repositório `fcg-solidarity-infra`, que concentra os componentes compartilhados e os manifests Kubernetes integrados.

Passo a passo esperado:

1. Clonar todos os repositórios da Fase 05.
2. Entrar no repositório `fcg-solidarity-infra`.
...
- O cliente não chama o Keycloak diretamente; ele usa a fachada `fcg-identity`.
...
- [Modelo da identidade](./architecture/fcg-identity-model.md)
...
- [Modelo de campanhas](./architecture/fcg-campaigns-model.md)
- [Endpoints](./architecture/endpoints.md)
...
- A `fcg-donations` consulta a `fcg-campaigns` para validar se a campanha está apta a receber doação.
...
- [Fluxo de doações](./architecture/fcg-donations-flow.md)
...
```text
Doador -> fcg-donations -> Kafka topic donation-received -> fcg-donation-worker -> fcg-campaigns
```
...
- O worker chama endpoint interno da `fcg-campaigns`.
- A `fcg-campaigns` atualiza `TotalAmountRaised` de forma idempotente.
...
- Componentes compartilhados ficam em `fcg-infra`.
...
- [Repositórios e infraestrutura](./architecture/repositories-and-infra.md)
- [ADR 0022 - Reutilizar fcg-pipelines](./adr/0022-reuse-fcg-pipelines-for-ci-cd.md)
...
- `fcg-audit-logs` com testes unitários.
- `fcg-solidarity-web` com testes unitários Angular/Vitest.
- Workflows reutilizáveis do `fcg-pipelines`.
...
| Database      | Dono                                    | Uso                                                  |
| ------------- | --------------------------------------- | ---------------------------------------------------- |
| `IdentityDb`  | `fcg-identity`                          | Perfis de domínio de Doador e GestorONG.             |
| `CampaignsDb` | `fcg-campaigns`                         | Campanhas e doações refletidas por campanha.         |
| `DonationsDb` | `fcg-donations` e `fcg-donation-worker` | Intenções de doação, outbox e mensagens processadas. |
...
- `fcg-donations` usa Transactional Outbox para garantir publicação confiável de `DonationReceivedEvent`.
- `fcg-campaigns` usa tabela de entradas processadas para refletir doações de forma idempotente.
...
| Database      | Dono             | Uso                                                       |
| ------------- | ---------------- | --------------------------------------------------------- |
| `AuditLogsDb` | `fcg-audit-logs` | Auditoria centralizada e imutável por eventos explícitos. |
...
- O `fcg-audit-logs` consome os eventos.
...
```mermaid
flowchart LR
    Web[fcg-solidarity-web] --> APIM[Azure API Management]
    APIM --> Identity[fcg-identity]
    APIM --> Campaigns[fcg-campaigns]
    APIM --> Donations[fcg-donations]
    ...
    Kafka --> Worker[fcg-donation-worker]
    Worker --> DonationsDb
    Worker --> Campaigns
    ...
    Worker --> AuditTopic
    AuditTopic --> AuditWorker[fcg-audit-logs]
...
| ---------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------- |
| JWT                                                  | Atendido                                              | `fcg-identity` + Keycloak                         |
| RBAC                                                 | Atendido                                              | Roles `GestorONG` e `Doador`                      |
| Endpoints administrativos para GestorONG             | Atendido                                              | `fcg-campaigns`                                   |
| Gestão de campanhas                                  | Atendido                                              | `fcg-campaigns`                                   |
| Cadastro público de Doador                           | Atendido                                              | `fcg-identity`                                    |
| Painel público de transparência                      | Atendido                                              | `fcg-campaigns` + `fcg-solidarity-web`            |
| Doação por Doador logado                             | Atendido                                              | `fcg-donations`                                   |
| Bloqueio de doação para campanha concluída/cancelada | Atendido                                              | `fcg-donations` + `fcg-campaigns`                 |
| Mínimo de dois microsserviços                        | Atendido                                              | Plataforma possui múltiplos serviços              |
| Comunicação assíncrona                               | Atendido                                              | Kafka + `fcg-donation-worker`                     |
| API não atualiza valor arrecadado diretamente        | Atendido                                              | Atualização feita pelo worker via `fcg-campaigns` |
| Kubernetes                                           | Atendido                                              | Kind local e AKS                                  |
| Deployments, Services e ConfigMaps                   | Atendido                                              | `fcg-solidarity-infra` e manifests das aplicações |
| `/health` ou `/metrics`                              | Atendido                                              | Serviços .NET                                     |
| Grafana com métricas reais                           | Atendido                                              | Prometheus + Grafana                              |
| Pipeline a cada push na principal                    | Atendido                                              | GitHub Actions via `fcg-pipelines`                |
